...
| Forwarded-header spoofing or incorrect public exposure | Host/origin bypass | Environment-owned canonical HTTPS identity, explicit trusted-proxy CIDRs, restricted HTTP proxy ingress, proxy shared secret, origin/host enforcement, Caddy security headers, and host-console recovery | A real reverse-proxy deployment validation remains a release gate before internet exposure. |
