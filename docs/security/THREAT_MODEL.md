...
| Forwarded-header spoofing or incorrect public exposure | Host/origin bypass | Canonical HTTPS identity, explicit trusted-proxy CIDRs, proxy shared secret, origin/host enforcement, Caddy security headers, staged/tested network changes and rollback | A real Cloudflare/Proxmox deployment validation remains a release gate before internet exposure. |
